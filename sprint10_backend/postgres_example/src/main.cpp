...
int main(int argc, const char* argv[]) {
    try {
        ...
        // Найдём фильм 2004 года. Мы уверены, что он есть в базе.
        {
            ...
            auto [id, title] =
                r.query1<int, std::string>("SELECT id, title FROM movies WHERE year=2004 LIMIT 1;"_zv);
        ...
        // Поищем фильм 1999 года. Мы не уверены, что он есть в базе.
        {
            ...
            std::optional result =
                r.query01<int, std::string>("SELECT id, title FROM movies WHERE year=1999 LIMIT 1;"_zv);
