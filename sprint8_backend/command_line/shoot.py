...
def check_prereqs() -> None:
    if shutil.which("perf") is None:
        raise SystemExit("perf not found in PATH. Install linux-tools or add perf to PATH.")
    if not (FLAMEGRAPH_DIR / "stackcollapse-perf.pl").exists():
        raise SystemExit(f"Cannot find {FLAMEGRAPH_DIR/'stackcollapse-perf.pl'}")
    if not (FLAMEGRAPH_DIR / "flamegraph.pl").exists():
        raise SystemExit(f"Cannot find {FLAMEGRAPH_DIR/'flamegraph.pl'}")
